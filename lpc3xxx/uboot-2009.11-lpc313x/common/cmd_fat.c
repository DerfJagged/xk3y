...
int do_fat_fsload (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (argc < 5) {
		printf ("usage: fatload <interface> <dev[:part]> <addr> <filename> [bytes] [enc]\n");
		return 1;
	}
	...
	if (argc == 6)
		...
		count = 0;
	size = file_fat_read (argv[4], (unsigned char *) offset, count, (argc == 7 && *argv[6] == '1'));
...
U_BOOT_CMD(
	...
	"<interface> <dev[:part]>  <addr> <filename> [bytes] [enc]\n"
