...
static uint32_t indir1_block[0x1000];
static int indir1_size = 0;
static int indir1_blkno = -1;

static uint32_t indir2_block[0x1000];
...
static int ext2fs_blockgroup
	...
	return (ext2fs_devread (blkno << LOG2_EXT2_BLOCK_SIZE(data),
		blkoff, sizeof(struct ext2_block_group), (char *)blkgrp, 0));
...
static int ext2fs_read_inode
	...
	status = ext2fs_devread (blkno << LOG2_EXT2_BLOCK_SIZE (data), blkoff,
				 sizeof (struct ext2_inode), (char *) inode, 0);
	if (status == 0) {
		return (0);
	}
...
int ext2fs_read_file
	(ext2fs_node_t node, int pos, unsigned int len, char *buf, int enc) {
	int i;
	int blockcnt;
	...
	for (i = pos / blocksize; i < blockcnt; i++) {
		...
		if (blknr)
		{
			...
			if (blknr == chunk.blknr + chunk.blkcnt)
				chunk.blkcnt++;

			if (i == blockcnt-1 || blknr != chunk.blknr + chunk.blkcnt - 1)
			{
				int bytes = blockend + (chunk.blkcnt-1)*blocksize - chunk.offset;
				int status;
				
				status = ext2fs_devread (chunk.blknr << log2blocksize, 
					chunk.offset, bytes, chunk.buf, enc);
...
int ext2fs_read (char *buf, unsigned len, int enc) {
	int status;
	...
	status = ext2fs_read_file (ext2fs_file, 0, len, buf, enc);
	return (status);
}
