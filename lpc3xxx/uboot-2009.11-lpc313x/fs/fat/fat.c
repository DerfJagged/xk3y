...
static int disk_read (__u32 startblock, __u32 getsize, __u8 * bufptr, int enc)
{
	...
	if (cur_dev->block_read) {
		return cur_dev->block_read (cur_dev->dev
			, startblock, getsize, (unsigned long *)bufptr, enc);
	}
	return -1;
}
...
fat_register_device(block_dev_desc_t *dev_desc, int part_no)
{
	...
	if (dev_desc->block_read (dev_desc->dev, 0, 1, (ulong *) buffer, 0) != 1) {
...
get_fatent(fsdata *mydata, __u32 entry)
{
	...
	if (bufnum != mydata->fatbufnum) {
		...
		if (disk_read(startblock, getsize, bufptr, 0) < 0) {
 ...
 */
static int
get_cluster(fsdata *mydata, __u32 clustnum, __u8 *buffer, unsigned long size, int enc)
{
	int idx = 0;
	__u32 startsect;
	...
	if (disk_read(startsect, size/FS_BLOCK_SIZE , buffer, enc) < 0) {
	...
	if(size % FS_BLOCK_SIZE) {
		...
		if (disk_read(startsect + idx, 1, tmpbuf, enc) < 0) {
			FAT_DPRINT("Error reading data\n");
			return -1;
		}
	     ...
	     unsigned long maxsize, int enc)
{
	...
	do {
		...
		if (get_cluster(mydata, curclust, buffer, (int)actsize, enc) != 0) {
		...
		buffer += actsize;
		actsize= filesize;
		if (get_cluster(mydata, endclust, buffer, (int)actsize, enc) != 0) {
			FAT_ERROR("Error reading cluster\n");
			return -1;
		}
		gotsize+=actsize;
	     ...
	     int dols, int enc)
{
	...
	if (disk_read (cursect, mydata->clust_size, do_fat_read_block, 0) < 0) {
  ...
  rootdir_done:
    ...
    ret = get_contents (mydata, dentptr, buffer, maxsize, enc);
    FAT_DPRINT ("Size: %d, got: %ld\n", FAT2CPU32 (dentptr->size), ret);

    /* make sure we have more than one block of zeroes at the end */
    if (enc)
    {
        long *l = (long*)&((char*)buffer)[(ret & ~3)-32];
        int i;
...
long
file_fat_read(const char *filename, void *buffer, unsigned long maxsize, int enc)
{
	printf("reading %s%s\n", enc?"and decrypting ":"", filename);
	return do_fat_read(filename, buffer, maxsize, LS_NO, enc);
}
