...
static unsigned int round_key[44];

static void encrypt_block(unsigned int block[], unsigned int const rk[]);
static void expand_key(unsigned int const key[], unsigned int round_key[]);

static int get_anticlone_data(unsigned int const serial[], unsigned int out[])
{
	unsigned int block[] = {
		ac_plaintext[0] ^ serial[0], ac_plaintext[1] ^ serial[0],
		ac_plaintext[2] ^ serial[0], ac_plaintext[3] ^ serial[0]
	};
		
	expand_key(ac_aeskey, round_key);
...
int factory_program_otp(void)
{
	struct otp new_otp = {

		.revocount	= (1 << MAX_REVO_COUNT)-1,
#ifdef FACTORY_DEVELOPMENT_BUILD
		.devclass	= OTP_DEVCLASS_DEVELOPMENT,
#else
		.devclass	= OTP_DEVCLASS_PRODUCTION,

		.bootkey	= {
		#include "boot_key.h"
		},
		.hwversion	= 1,
#ifdef FACTORY_CHINA_ONLY
		.chineselanguageonly = 1,
#endif
		.dfu_boot	= 1,
		.aes_key_valid	= 1,
		.jtag_sec	= 7,
	...
	extern unsigned long ext_random[];

#ifndef FACTORY_DEVELOPMENT_BUILD
	/* don't clobber the serial or devclass if it's already there */
	if (!OTP.serial[0])
	{
		new_otp.serial[0] = ext_random[0];
		get_anticlone_data(new_otp.serial, new_otp.acdata);
	}
#endif
